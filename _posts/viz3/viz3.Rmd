...
Here are the libraries needed for this plot: 
...
That is better, but improvements could also be made in overall readability (e.g. due to plot dimensions, it's hard to tell which dot-line belongs to which county) and other aspects of tidying (e.g. title case for county names). I also tried playing with the color palette.  
...
I also made the scale colors display in the subtitle rather than in a legend.  
...
Although this works, I wanted to highlight the counties which saw **increases** in scores vs. the counties that saw **decreases** in scores. This is the main point of the plot and should be immediately evident to the viewer. To do this, I added another `geom_line` for only those counties that saw decreases. I also reversed the `fct_reorder` so that counties with increases were at the top and counties with decreases were at the bottom.    

```{r preview=TRUE, fig.height=5, fig.width=5, layout="l-body-outset"}
